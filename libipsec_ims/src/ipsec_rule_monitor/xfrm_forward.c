...
int getifbyaddr(char *ifname, char *addr, __u8 prefix_len, __u16 family)
{
	...
	if((family == AF_INET && prefix_len == IPV4_PREFIX_LEN) ||
		...
		if(getifaddrs(&ifa_list) < 0) {
			...
			strncpy(ifname, IF_HARD_CODE, strlen(IF_HARD_CODE));
			return 1;
		}
	...
	if(enable) {
		...
		if(forward_rule_count() == 1) {
			...
			if(ifc_set_ipsec_forward(interface, "wlan0", dst, pref_d, enable) == 0)
				ALOGD("enable forward success\n");
		}

		ifr_ccmni_fwd_filter.action = CCMNI_FLT_ADD;
		if(ioctl(ifc_ctl_sock, SIOCFWDFILTER, &ifr) < 0) {
			ALOGE("enable_interface_forward ioctl %d failded, %s\n", SIOCFWDFILTER, strerror(errno));
			close(ifc_ctl_sock);
			return -errno;
		}
		ALOGD("ioctl: src:%s/%u,dst:%s/%u,family:%u from %s should be forwarded\n",src, pref_s, dst,pref_d, family, interface);
		...
		if(forward_rule_count() == 0) {
			...
			if(ifc_set_ipsec_forward(interface, "wlan0",dst, pref_d,  enable) == 0)
				ALOGD("disable forward success\n");
		}

		ifr_ccmni_fwd_filter.action = CCMNI_FLT_DEL;
		if(ioctl(ifc_ctl_sock, SIOCFWDFILTER, &ifr) < 0) {
			ALOGE("disable_interface_forward ioctl failded, %s\n", strerror(errno));
			close(ifc_ctl_sock);
			return -errno;
		}
		ALOGD("ioctl: src:%s/%u,dst:%s/%u,family:%u from %s should not be forwarded\n",src, pref_s, dst,pref_d, family, interface);
	}
