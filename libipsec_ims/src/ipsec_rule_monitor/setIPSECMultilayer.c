...
char *rt_addr_ntoa(int af,const void *addr, char *buf, int buflen)
{
	switch (af) {
	case AF_INET:
	case AF_INET6:
...
char *rt_addr_aton(int af,const void *srcaddr, char *dstbuf)
{
	switch (af) {
	case AF_INET:
	case AF_INET6:
...
int deleteRuledatabase(siptx_req_ipsec_connect_struct * Ruledatabase,struct xfrm_userpolicy_id *xpid,char * srcbuf, char * dstbuf)
{
        ...
        for(i =0; i<RECORD_COUNT;i++)
	{
	    ...
	    if((strcmp(tmp->local_ip_c,srcbuf)==0)&&(strcmp(tmp->target_ip_s,dstbuf)==0)&&(tmp->proto == xpid->sel.proto) && xpid->sel.prefixlen_s == tmp->pref_s && xpid->sel.prefixlen_d == tmp->pref_d && xpid->dir == tmp->dir && tmp->local_port_c == ntohs(xpid->sel.sport) && tmp->target_port_s == ntohs(xpid->sel.dport))
	    {
			...
			/*sony project && out, 2layer policy will be set, enable ccmni4 forward once*/
			if(sony_project && tmp->dir == XFRM_POLICY_OUT)
			{
				char ifname[IFNAMSIZ] = {0};
				if(getifbyaddr(ifname, tmp->local_ip_c, tmp->pref_s, tmp->family))
					set_interface_forward(tmp->local_ip_c, tmp->pref_s, tmp->target_ip_s,tmp->pref_d, tmp->family, ifname, OFF);
				else
...
void set2layeripsecrules_xfrm(siptx_req_ipsec_connect_struct * Tunndatabse,siptx_req_ipsec_connect_struct Transdata_var,__u8 dir,int Handover)
{
    ...
    for(i =0; i<RECORD_COUNT;i++)
	{
		...
		switch(dir)
		{
			...
			if(!Handover) {
				//+ 20170207 LloydLin - Emergency call via IMS - Fix DMS10927049
        	...
        	/*sony project && out, 2layer policy will be set, enable ccmni4 forward once*/
		ALOGD("check ccmni forwarding, project %d, dir %d, handover %d", sony_project, tmp->dir, Handover);
		if(sony_project && tmp->dir == XFRM_POLICY_OUT)
		{
			char ifname[IFNAMSIZ] = {0};
			if(!Handover) {
				if(getifbyaddr(ifname, Transdata_var.local_ip_c, Transdata_var.pref_s, Transdata_var.family))
					set_interface_forward(Transdata_var.local_ip_c, Transdata_var.pref_s, Transdata_var.target_ip_s,Transdata_var.pref_d, Transdata_var.family, ifname, ON);
				else
					ALOGE("getifbyaddr failed, interface cannot forward\n");
				continue;
			} else {
				if(getifbyaddr(ifname, tmp->local_ip_c, tmp->pref_s, tmp->family))
					set_interface_forward(tmp->local_ip_c, tmp->pref_s, tmp->target_ip_s,tmp->pref_d, tmp->family, ifname, ON);
				else
					ALOGE("getifbyaddr failed, interface cannot forward\n");
				continue;
			}
		...
		tmpls_len += sizeof(*tmpl);
		if (tmpls_len > 0) {
			addattr_l(&req.n, sizeof(req), XFRMA_TMPL, (void *)tmpls_buf, tmpls_len);
		}	
...
void change1layeripsecrules_xfrm(siptx_req_ipsec_connect_struct * Transdatabase,struct xfrm_userpolicy_id *xpid, char * srcbuf, char * dstbuf)
{
	...
	for(i =0; i<RECORD_COUNT;i++)
	{
		...
		/*sony project && out, 1layer policy will be set, disable ccmni4 forward*/
		if(sony_project && tmp->dir == XFRM_POLICY_OUT)
		{
			char ifname[IFNAMSIZ] = {0};
			if(getifbyaddr(ifname, tmp->local_ip_c, tmp->pref_s, tmp->family))
