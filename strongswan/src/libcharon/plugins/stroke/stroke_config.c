...
static ike_cfg_t *build_ike_cfg(private_stroke_config_t *this, stroke_msg_t *msg)
{
	...
	msg->add_conn.me.ikeport = get_ut_ikev2_udp_port();
	msg->add_conn.me.ikeport_natt = get_ut_ikev2_udp_natt_port();
#endif
								  ...
								  stroke_msg_t *msg, ike_cfg_t *ike_cfg)
{
	...
	*/
	...
	/*
	check whether enable pcscf restore feature
	*/
	if(msg->add_conn.pcscf_restore)
	{
		peer_cfg->set_pcscf_restore(peer_cfg);
		DBG1(DBG_CFG, "enable pcscf retore");
	} 

	/*
	check whether enable emergency indication feature
	*/
	if(msg->add_conn.is_emergency)
	{
		peer_cfg->set_is_emergency(peer_cfg);
		DBG1(DBG_CFG, "enable emergency indication");
	} 

	/*
	check whether enable device_identity feature
	*/
	if(msg->add_conn.device_identity)
	{
		peer_cfg->set_device_identity(peer_cfg);
		DBG1(DBG_CFG, "enable emergency indication");
	} 
