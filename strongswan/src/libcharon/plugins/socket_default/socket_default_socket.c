...
int IKEV2_UDP_PORT = 1500;
int CHARON_UDP_PORT = 1500;
int IKEV2_NATT_PORT = 4500;
int CHARON_NATT_PORT = 4500;
#endif
					   ...
					   int family, u_int16_t *port)
{
	...
	{
		int retVPN = 0;
		retVPN = protectFromVpn(skt);
		if (retVPN) {
			DBG1(DBG_NET, "protectFromVpn socket error: %s, %d", strerror(errno), retVPN);
		}
		DBG1(DBG_NET, "protectFromVpn skt: %d, DONE", skt);
	}
							...
							int *skt, int *skt_natt, char *label)
{
...
#ifdef STRONGSWAN_UNIT_TEST
	for(int i=this->port; i< this->port+200;i++)
	{
		DBG1(DBG_NET, "open_socketpair ut_ike_port :%i", this->port);
	...
	for(int i=this->natt; i< this->natt+200;i++)
	{
		DBG1(DBG_NET, "open_socketpair ut_natt_port :%i", this->natt);
	...
	set_ut_ikev2_udp_port(this->port);
	set_ut_ikev2_udp_natt_port(this->natt);
