...
static status_t parse(private_encrypted_payload_t *this, chunk_t plain)
{
	...
	parser->set_major_version(parser, this->type == ENCRYPTED_V1 ? 1 : 2);
...
/* for some reason iv.len is 0 at this point in UT. Need to be fixed properly */
	iv.len=16;
#endif
														 ...
														 chunk_t plain)
{
	...
	this = (private_encrypted_payload_t*)encrypted_payload_create(ENCRYPTED);
...
{
	if (this->fragment_number == 1 && this->next_payload == NO_PAYLOAD)
	{
		this->next_payload = type;
	}
}
