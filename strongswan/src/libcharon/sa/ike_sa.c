...
METHOD(ike_sa_t, set_oos, void,
	private_ike_sa_t *this, bool oos)
{
	this->oos = oos;
	if (oos)
	{
		notify_wod(N_OOS_START, get_name(this) + 4, NULL);
		this->oos_start_time = time_monotonic(NULL);
	}
	else
	{
		notify_wod(N_OOS_END, get_name(this) + 4, NULL);
	}
}
...
METHOD(ike_sa_t, get_backoff_time, int,
	private_ike_sa_t *this)
{
	...
	private_ike_sa_t *this, message_t *message)
{
	...
	{
		notify_wod_attach_failed(get_name(this) + 4,
	...
	private_ike_sa_t *this, bool address)
{
	...
	if (!is_any_path_valid(this))
	{
		...
		{
			job_t *job;
			DBG1(DBG_IKE, "============================= oos enabled: %d", oos_timeout);
			set_oos(this, TRUE);
			job = (job_t*)send_dpd_job_create(this->ike_sa_id);
						 ...
						 ike_version_t version)
{
	...
	INIT(this,
		.public = {
			...
			.get_backoff_time = _get_backoff_time,
