		...
		array_count(this->initiating.packets))
	{
		...
		if (!mobike || !mobike->is_probing(mobike))
		{
			...
			if (this->initiating.retransmitted > 0)
			{
				...
				if (!src->is_anyaddr(src) && hydra->kernel_interface->get_interface(hydra->kernel_interface, src, &iface) == FALSE)
				{
...
RETRANSMIT_FAILED:
	...
	{
		this->ike_sa->set_oos(this->ike_sa, FALSE);
		DBG1(DBG_IKE, "============================= Retransmit DPD timeout, oos disabled.");
	}
	return DESTROY_ME;
}
